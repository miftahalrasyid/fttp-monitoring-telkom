...
// const DOMPurify = dynamic(() => import("dompurify"), { ssr: false }) as any;
...
function Viewonly_ODC({
  ...
  const createMarkup = (html) => {
    console.log("markup", html, DOMPurify)
                  ...
                  {panel.data.map((r_item, idx) => {
                    ...
                    return <Rak key={'r' + r_item.rak_level} distributor_level_id={r_item.rak_index} last_feeder={panel.data.filter(item => item.type === "feeder").length} level={r_item.rak_level} type={r_item.type} datalen={12}>
                      {r_item.data.map(p_item =>
                        ...
                        <Eth from={r_item.type} clickHandler={panelClickHandler} key={"port" + p_item.index} rak_level={r_item.rak_level} id={p_item.index} status={p_item.status} rak_index={r_item.rak_index}
                          panel={panel}
                    ...
                    <Box itemRef='testing' sx={{
                      ...
                      <div className={`${styles.card}  ${styles.cardStats}`}>
                        ...
                        <div className={`${styles.cardBody} card-body row`} style={{ width: "100%" }}>
                          {/* <Editor
                            editorStyle={{ height: "47vh", border: "1px solid #F1F1F1", maxWidth: "100%" } as any}
                          ...
                          /> */}
                          {(serverNotes || false) ? <div className="preview" style={{ height: "47vh", border: "1px solid #F1F1F1", maxWidth: "100%" }} dangerouslySetInnerHTML={createMarkup(serverNotes)}></div> : <p style={{ height: "47vh", border: "1px solid #F1F1F1", textAlign: "center", margin: "auto", maxWidth: "100%" }}>Tidak ada data</p>}
                        </div>

...
import {
  ...
  ButtonProps,
  Checkbox,
  FormControl, InputLabel, InputLabelProps, ListItemText, MenuItem, Select, TabsProps, TextFieldProps
} from '@mui/material'
...
function ODC(props) {
  ...
  const graph = {
    feeder: {
      ...
      options: {
        ...
        plotOptions: {
          bar: {
            horizontal: (global.innerHeight > global.innerWidth) ? true : false,
            borderRadius: 10
          },
        },
        xaxis: {
  ...
  const [newTableState, setNewTableState] = useState({ page: 0, rowsPerPage: 5, search_text: null, sort: { orderBy: null, direction: null }, filter: new Array(24) })
  ...
  return (
    <div className={styles.mainContent}>
      ...
      <div key={"charts"} className={styles.charts}>
        <div>
          <h4 ref={feederChartRef} style={{ textTransform: "capitalize" }}>{`Feeder Mapping `}</h4>
          {((graph.feeder?.series[0]?.data || false) || (graph.feeder?.series[0]?.data?.length === 0)) ? <ApexChart
            options={graph.feeder.options} series={graph.feeder.series} type="bar" width={Math.min(600, global.innerWidth * 0.85)} height={350}
          /> : <h2>No Data</h2>}
        </div>
        <div>
          <h4 ref={distribusiChartRef} style={{ textTransform: "capitalize" }}>Distribution Mapping</h4>
          {((graph.distribution?.series[0]?.data || false) || (graph.distribution?.series[0]?.data?.length === 0)) ? <ApexChart
            options={graph.distribution.options} series={graph.distribution.series} type="bar" width={Math.min(600, global.innerWidth * 0.85)} height={350}
          /> : <h2>No Data</h2>}
        </div>
      </div>,
          ...
          {datatable ? <DynamicMUIDataTable
            ...
            options={{
              ...
              onTableInit: (test, tableState) => {
                ...
                changeODCPage({ page: tableState.page + 1, rowsPerPage: tableState.rowsPerPage, region: submittedFilter.regional, witel: submittedFilter.witel, datel: submittedFilter.datel, sto: submittedFilter.sto, sortBy: null, sortOrder: null, name: null, filter: tableState.filterList }, token, toast)
              },
              onTableChange: (action, tableState, index) => {
                // console.log(action, tableState,index);
                ...
                switch (action) {
                  ...
                  case "search":
                    ...
                    delay.current = setTimeout(() => {
                      changeODCPage({ page: 0, rowsPerPage: newTableState.rowsPerPage, region: submittedFilter.regional, witel: submittedFilter.witel, datel: submittedFilter.datel, sto: submittedFilter.sto, sortBy: newTableState.sort.orderBy, sortOrder: newTableState.sort.direction, name: tableState.searchText, filter: newTableState.filter }, token, toast)
                    }, 500)
                  ...
                  case 'sort':
                    ...
                    changeODCPage({ page: 0, rowsPerPage: newTableState.rowsPerPage, region: submittedFilter.regional, witel: submittedFilter.witel, datel: submittedFilter.datel, sto: submittedFilter.sto, sortBy: sortConvention, sortOrder: tableState.sortOrder.direction.toLocaleUpperCase(), name: newTableState.search_text, filter: newTableState.filter }, token, toast)
                    // this.sort(tableState.page, tableState.sortOrder);
                    break;
                  case "filterChange":
                    // console.log("filter change",tableState.filterList)
                    setNewTableState(prev => ({ ...prev, page: 0, filter: tableState.filterList }))
                    changeODCPage({ page: 0, rowsPerPage: newTableState.rowsPerPage, region: submittedFilter.regional, witel: submittedFilter.witel, datel: submittedFilter.datel, sto: submittedFilter.sto, sortBy: newTableState.sort.orderBy, sortOrder: newTableState.sort.direction, name: newTableState.search_text, filter: tableState.filterList }, token, toast)
                    break;
                  default:

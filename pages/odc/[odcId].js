...
function Odc({
    ...
    const selectedODC = (ODCDataClient[0]?.id?ODCDataClient:ODCData).filter(dt=>dt.id==odcId)
    ...
    // },[onDataUpdate])
    // useEffect(()=>{
    //     console.log("distri state",splitterData.filter(item=>item.id==distributeState?.isOnClick?.ids[0]%24 && item.ds==Math.ceil(distributeState?.isOnClick?.ids[0]/24))[0])
    // },[distributeState])
    // useEffect(()=>{
    //     console.log("distri state",feederDetail)
    // },[feederDetail])

...
function Modal(props) {
    ...
    if (distributionOnChange[poid].selected.id !== "") {
      ...
      setDistributionOnChange(prev => prev.map((item, idx) => {
        if (idx !== poid) {
          ...
          return {
            mapped: item.mapped.map((rak, idx1) => {
          ...
          return {
            mapped: item.mapped, selected: (ev.target.value == 0) ? { id: "", index: "", status: "", rak_index: "", rak_level: "" } : item.mapped.map(rak => {
  ...
  const handleSplitterChange = (ev, setValues) => {
    if (ev.target.value == "" || (feederFocus.splitter?.splitter_id || false)) {
      // if (ev.target.value == "") {
      ...
      <div>
        ...
        <Box itemRef='detailFeederModal' sx={{
          ...
          <div className={`${odcStyles.card}  ${odcStyles.cardStats}`}>
            ...
            <Formik
              initialValues={{
                // splitter: (feederFocus.splitter?.splitter_id || false) ? (feederFocus.splitter.splitter_id ? feederFocus.splitter.splitter_id : splitterData?.filter(item => item.status === "idle")[0]?.id) : "",
                splitter: (feederFocus.splitter?.splitter_id ? feederFocus.splitter.splitter_id : (feederFocus.portType == "idle" ? splitterData?.filter(item => item.status === "idle")[0]?.id : "")),
              ...
              validate={(values) => {
                if ((feederFocus.splitter?.splitter_id || false) || feederFocus.portType === 'idle') {
                      ...
                      <div className={`row ${odcStyles.formGap}`}>
                        ...
                        <div className={`col-lg-9 col-md-6 ${styles.textFieldContainer} ${styles.splitter}`}>
                          <CustomFormControl sx={{ pointerEvents: router.asPath.split("/")[1] == 'view' ? "none" : "all" }} label={"splitter"} key='splitter' variant="standard" >
                            {router.asPath.split("/")[1] == 'view' ? <Input value={feederFocus?.splitter?.splitter_index || 'Empty'}></Input>
                            ...
                            {router.asPath.split("/")[1] == 'view' ? <Input value={(feederFocus?.distribution[0]?.distribution_level || false) ? (feederFocus?.distribution[0].distribution_level % 2 == 0) ? "D" + feederFocus?.distribution[0]?.distribution_level_id + "-" + (feederFocus.distribution[0].distribution_index + 12) : "D" + feederFocus?.distribution[0]?.distribution_level_id + "-" + feederFocus.distribution[0].distribution_index : "Empty"}></Input>
                              : [<NativeSelect key={'natsel1'} /*onChange={po1ClickHandler}*/ onChange={(ev) => onchg1(ev, 0, setValues)} onBlur={handleBlur} value={values.dist_port_1} inputProps={{
                                ...
                                <option value={""}>Empty</option>
                                {
                                  ...
                                  <option key={"dist_po1" + feederFocus.distribution[0].distribution_index} value={feederFocus.distribution[0].distribution_id}>{(feederFocus?.distribution[0].distribution_level % 2 == 0) ? "D" + feederFocus?.distribution[0]?.distribution_level_id + "-" + (feederFocus.distribution[0].distribution_index + 12) : "D" + feederFocus?.distribution[0]?.distribution_level_id + "-C" + feederFocus.distribution[0].distribution_index}</option>
                                }
